   function addli(){
     let inp = document.querySelector("textarea").value.split(',')
     let ul = document.querySelector("ul")
     inp.forEach(element => {
       let li = document.createElement("li")
       ul.innerHTML+=`<li data-block=${element} class="block" data-width="">${element}</li>`
     });
     insetup();
   }
   var list     = null
  ...
  function AddtoList(){
   ...
   let nums=document.getElementsByName(inputnum)
console.log(nums)
    var val = document.createTextNode(nums);
   //  node.data-block=val;     
